...
func (s *SmartContract) updateInvoice(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	invoice := Invoice{}
	err = json.Unmarshal(invoiceAsBytes, &invoice)
	invoice.LegalOrgID, _ = strconv.Atoi(args[1])
	invoice.LegalOwnerName = args[2]
	invoice.Obligator = args[3]
	invoice.DiscountRate, _ = strconv.ParseFloat(args[4], 64)
	invoice.NotionalDiscount, _ = strconv.ParseFloat(args[5], 64)
	invoice.AssetStatus = args[6]
	invoice.LastUpdateDate = args[7]
	invoice.LastUpdatedBy = args[8]
	invoice.DiscountedPayableAmt, _ = strconv.ParseFloat(args[9], 64)
	invoice.AppliedDiscountRateAbsolute, _ = strconv.ParseFloat(args[10], 64)
	invoice.AppliedDiscountRateLIBORBased, _ = strconv.ParseFloat(args[11], 64)
	invoice.FincoOrgID, _ = strconv.Atoi(args[12])
	invoice.FincoOrgName = args[13]
	invoice.TotalAmountToBePaidToSupplier, _ = strconv.ParseFloat(args[14], 64)
